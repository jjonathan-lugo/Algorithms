...
struct connection 
{
    string name_road, type_road, direction;
    int A_start, B_end;
    double length;
...
struct intersection 
{
    int intersection_number, previous_intersection, previous_connection_index;
    double longitude, latitude, distance_nearest, dist_from_source;
    string states_abv, named_places;
...
struct Place 
{
    int numeric_code, population, intersection_code;
    string state_abbreviation, name;
    double area, latitude, longitude, distance_intersection;
    Place() : numeric_code(0), state_abbreviation(""), name(""), population(0), 
              area(0.0), latitude(0.0), longitude(0.0), intersection_code(0), distance_intersection(0.0) {}

    Place(int nc, string state_abbr, string n, int pop, double ar, double lat, double lon, int ic, double dist) :
        numeric_code(nc), state_abbreviation(state_abbr), name(n), population(pop), 
        area(ar), latitude(lat), longitude(lon), intersection_code(ic), distance_intersection(dist) {}
};
...
struct PathStep
{
    ...
    string road_name, road_type, direction, destination_place_name, destination_place_state_abv;
    double distance;
    PathStep() : from_intersection(-1), to_intersection(-1), road_name(""), road_type(""), 
                 distance(0.0), direction(""), destination_place_name(""), destination_place_state_abv("") {}
};
...
struct dat_file 
{
    ...
    dat_file() : filename(""), min_lat(0), max_lat(0), min_lon(0), max_lon(0) {}

    dat_file(double minlat, double maxlat, double minlon, double maxlon, string fname) :
        min_lat(minlat), max_lat(maxlat), min_lon(minlon), max_lon(maxlon), filename(fname) {}
};

struct dat_data 
{
    int rows, columns, bytes_per_pixel, min_elevation, max_elevation, special_val;
    double seconds_per_pixel, left_lon_seconds, top_lat_seconds;
    short** elevation_data;
    dat_data() : rows(0), columns(0), bytes_per_pixel(0), seconds_per_pixel(0), left_lon_seconds(0), top_lat_seconds(0),
                min_elevation(0), max_elevation(0), special_val(0), elevation_data(nullptr) {}
    ~dat_data() 
    {
        if (elevation_data) 
        {
            for (int i = 0; i < rows; i++) 
                delete[] elevation_data[i];
...
State parse_state_line(const string& line)
{
    ...
    if (space_pos == string::npos) 
        return State();
    return State(line.substr(0, space_pos), line.substr(space_pos + 1));
}
...
{
    double lat_diff = end.latitude - start.latitude, lon_diff = end.longitude - start.longitude;
    string direction = "";
    if (lat_diff > 0) 
        direction += "N";
    else if (lat_diff < 0)
...
void add_reverse_connections(vector<intersection>& intersections_vec)
{
    ...
    for (size_t i = 0; i < intersections_vec.size(); i++) 
    {
        for (size_t j = 0; j < intersections_vec[i].leading_roads.size(); j++) 
        {
            ...
            int from = conn.A_start, to = conn.B_end;
...
void resize_hash_table(HashTable& table)
{
    ...
    int old_size = table.size, new_size = old_size * 2;
...
Place parse_place_line(const string& line) 
{
    ...
    while (*p && ((*p >= '0' && *p <= '9') || *p == '.'))
        p++;
    if (p > num_start) 
        area = stod(string(num_start, p - num_start));
    while (*p == ' ')
        p++;
    double latitude = 0.0;
    num_start = p;
    while (*p && ((*p >= '0' && *p <= '9') || *p == '.'))
        p++;
    if (p > num_start) 
        latitude = stod(string(num_start, p - num_start));
    while (*p == ' ') 
        p++;
    ...
    while (*p && ((*p >= '0' && *p <= '9') || *p == '.'))
        p++;
    if (p > num_start) 
        longitude = stod(string(num_start, p - num_start));
    while (*p == ' ') 
        p++;
    ...
    while (*p && ((*p >= '0' && *p <= '9') || *p == '.')) 
        p++;
    if (p > num_start) 
        distance = stod(string(num_start, p - num_start));
    return Place(numeric_code, state_abbr, place_name, population, area, latitude, longitude, intersection_code, distance);
}
...
{
    double angle = calculate_angle(start.latitude, start.longitude, end.latitude, end.longitude);
...
void display_state_options(const vector<Place>& places, const vector<State>& states_vec) 
{
    ...
    for (size_t i = 0; i < places.size(); i++)
    {
        ...
        if (!already_shown)
        {
            ...
            cout << displayed.size() + 1 << ". " << places[i].state_abbreviation << " " << full_name << "\n";
...
Place select_place_from_list(const vector<Place>& places, const vector<State>& states_vec) 
{
    ...
    for (size_t i = 0; i < places.size(); i++) 
    {
        ...
        if (!found) unique_states.push_back(places[i].state_abbreviation);
    }
...
bool get_place_with_state_hint(const string& user_input, HashTable& place_table, vector<State>& states_vec, Place& selected_place)
{
...
bool reconstruct_path(const vector<intersection>& intersections_vec, int start_index, int end_index, vector<PathStep>& path) 
{
...
void print_path(const vector<PathStep>& path, const Place& start_place, const Place& end_place) 
{
    cout << fixed << setprecision(2);
    cout << "Start at " << start_place.distance_intersection << " miles from " << start_place.name 
         << " " << start_place.state_abbreviation << "\n";
    ...
    for (size_t i = 0; i < path.size(); i++)
    {
        ...
        cout << "then take " << road_full << " " << step.distance << " miles " << step.direction << " to ";
    ...
    if (!path.empty())
        cout << "then continue " << end_place.distance_intersection << " miles to " << end_place.name
             << " " << end_place.state_abbreviation << "\n";
    cout << "\nTotal distance: " << total_distance << " miles.\n";
}

vector<dat_file> load_coverage_file()
{
    vector<dat_file> files;
    ifstream fin("coverage.txt");
    if (!fin.is_open())
    {
        cerr << "Error: Could not open coverage.txt\n";
        return files;
    }
    string line;
    while (getline(fin, line)) 
    {
        ...
        string fname;
        ss >> minlat >> maxlat >> minlon >> maxlon >> fname;
        files.push_back(dat_file(minlat, maxlat, minlon, maxlon, fname));
    }
    fin.close();
    return files;
}

string find_dat_file(double start_lat, double start_lon, double end_lat, double end_lon, const vector<dat_file>& coverage)
{
    if (start_lon > 180) start_lon -= 360;
    if (end_lon > 180) end_lon -= 360;
    double min_lat = (start_lat < end_lat) ? start_lat : end_lat;
    double max_lat = (start_lat > end_lat) ? start_lat : end_lat;
    double min_lon = (start_lon < end_lon) ? start_lon : end_lon;
    double max_lon = (start_lon > end_lon) ? start_lon : end_lon;
    string best_file = "";
    double best_area = 0;
    for (size_t i = 0; i < coverage.size(); i++) 
    {
        double file_min_lat = (coverage[i].min_lat < coverage[i].max_lat) ? coverage[i].min_lat : coverage[i].max_lat;
        double file_max_lat = (coverage[i].min_lat > coverage[i].max_lat) ? coverage[i].min_lat : coverage[i].max_lat;
        double file_min_lon = (coverage[i].min_lon < coverage[i].max_lon) ? coverage[i].min_lon : coverage[i].max_lon;
        double file_max_lon = (coverage[i].min_lon > coverage[i].max_lon) ? coverage[i].min_lon : coverage[i].max_lon;
        bool lat_overlap = !(max_lat < file_min_lat || min_lat > file_max_lat);
        bool lon_overlap = !(max_lon < file_min_lon || min_lon > file_max_lon);
        if (lat_overlap && lon_overlap) 
        {
            double file_area = (file_max_lat - file_min_lat) * (file_max_lon - file_min_lon);
            if (file_area > best_area)
            {
                best_area = file_area;
                best_file = coverage[i].filename;
            }
        }
    }
    return best_file;
...
bool load_dat_data(const string& filename, dat_data& dat) 
{
    ...
    for (int i = 0; i < 9; i++) 
    {
        ...
        if (s == "rows") 
            ...
            dat.bytes_per_pixel = val;
        else if (s == "secondsperpixel") 
            dat.seconds_per_pixel = val;
        else if (s == "leftlongseconds") 
            dat.left_lon_seconds = val;
        else if (s == "toplatseconds") 
            dat.top_lat_seconds = val;
        else if (s == "min") 
            dat.min_elevation = val;
        else if (s == "max") 
            dat.max_elevation = val;
        else if (s == "specialval") 
            dat.special_val = val;
    }
    fin.ignore(1000, '\n');
    dat.elevation_data = new short* [dat.rows];
    for (int i = 0; i < dat.rows; i++)
    {
        dat.elevation_data[i] = new short[dat.columns];
        fin.read((char*)dat.elevation_data[i], dat.columns * sizeof(short));
        if (!fin.good() && i < dat.rows - 1) 
        {
            cerr << "Error reading row " << i << " of elevation data\n";
            return false;
        }
    }
    fin.close();
...
void latlon_to_pixel(double lat, double lon, const dat_data& dat, int& px, int& py) 
{
    double lat_seconds = lat * 3600.0;
    double lon_seconds = lon * 3600.0;
    py = (int)((dat.top_lat_seconds - lat_seconds) / dat.seconds_per_pixel);
    px = (int)((lon_seconds - dat.left_lon_seconds) / dat.seconds_per_pixel);
    if (px < 0) 
        px = 0;
    if (px >= dat.columns) 
        px = dat.columns - 1;
...
int get_elevation_color(int elevation, int min_elev, int max_elev, int special_val) 
{
    if (elevation == special_val || elevation < 0) 
        return color::blue;
    ...
    if (range <= 0) 
        return color::green;
    double normalized = (elevation - min_elev) / range;
    double hue, lightness, saturation;
    hue = 120.0 - (normalized * 60.0);
    lightness = 0.4 + (normalized * 0.2);
    saturation = 0.5 - (normalized * 0.2);
    return color::hls(hue, lightness, saturation);
}

void draw_map(window* win, const dat_data& dat, int win_width, int win_height) 
{
    win->enter_fast_mode();
    win->fill_rectangle(0, 0, win_width, win_height, make_color(0.9, 0.9, 0.85));
    double scale_x = (double)win_width / dat.columns;
    double scale_y = (double)win_height / dat.rows;
    for (int row = 0; row < dat.rows; row += 2) 
    {
        for (int col = 0; col < dat.columns; col += 2)
        {
            int elev = dat.elevation_data[row][col];
            int x = (int)(col * scale_x);
            int y = (int)(row * scale_y);
            int w = (int)(2 * scale_x) + 1;
            int h = (int)(2 * scale_y) + 1;
            if (elev == dat.special_val || elev < -100) 
                win->fill_rectangle(x, y, w, h, make_color(0.6, 0.8, 1.0));
            else 
            {
                double norm = (elev - dat.min_elevation) / (double)(dat.max_elevation - dat.min_elevation);
                if (norm < 0) norm = 0;
                if (norm > 1) norm = 1;
                double r = 0.7 + norm * 0.2;
                double g = 0.85 - norm * 0.2;
                double b = 0.6 - norm * 0.3;
                win->fill_rectangle(x, y, w, h, make_color(r, g, b));
            }
        }
    }
    win->catch_up();
...
void draw_path_on_map(window* win, const vector<PathStep>& path, const vector<intersection>& graph_vec,
    const dat_data& dat, int win_width, int win_height) 
{
    win->set_pen_width(5);
    win->set_pen_color(color::red);
    double scale_x = (double)win_width / dat.columns;
    double scale_y = (double)win_height / dat.rows;
    bool first = true;
    for (size_t i = 0; i < path.size(); i++) 
    {
        int from = path[i].from_intersection, to = path[i].to_intersection;
        int px1, py1, px2, py2;
        latlon_to_pixel(graph_vec[from].latitude, graph_vec[from].longitude, dat, px1, py1);
        latlon_to_pixel(graph_vec[to].latitude, graph_vec[to].longitude, dat, px2, py2);
        int screen_x1 = (int)(px1 * scale_x), screen_y1 = (int)(py1 * scale_y);
        int screen_x2 = (int)(px2 * scale_x), screen_y2 = (int)(py2 * scale_y);
        if (first)
        {
            win->move_to(screen_x1, screen_y1);
            first = false;
        }
        win->draw_to(screen_x2, screen_y2);
    }
    win->catch_up();
}

void draw_markers(window* win, const Place& start_place, const Place& end_place,
    const dat_data& dat, int win_width, int win_height) 
{
    double scale_x = (double)win_width / dat.columns;
    double scale_y = (double)win_height / dat.rows;
    int px, py;
    latlon_to_pixel(start_place.latitude, start_place.longitude, dat, px, py);
    int sx = (int)(px * scale_x), sy = (int)(py * scale_y);
    win->set_pen_color(color::green);
    win->set_pen_width(12);
    win->draw_point(sx, sy);
    win->set_pen_color(color::black);
    win->fill_rectangle(sx + 10, sy - 25, 200, 20, color::white);
    win->move_to(sx + 15, sy - 10);
    win->write_string("START: " + start_place.name + ", " + start_place.state_abbreviation);
    latlon_to_pixel(end_place.latitude, end_place.longitude, dat, px, py);
    int ex = (int)(px * scale_x), ey = (int)(py * scale_y);
    win->set_pen_color(color::blue);
    win->set_pen_width(12);
    win->draw_point(ex, ey);
    win->set_pen_color(color::black);
    win->fill_rectangle(ex + 10, ey - 25, 200, 20, color::white);
    win->move_to(ex + 15, ey - 10);
    win->write_string("END: " + end_place.name + ", " + end_place.state_abbreviation);
    win->catch_up();
}

void visualize_path(const Place& start_place, const Place& end_place,
    const vector<PathStep>& path, const vector<intersection>& graph_vec)
{
    vector<dat_file> coverage = load_coverage_file();
    if (coverage.empty())
    {
        cerr << "Could not load coverage file. Skipping visualization.\n";
        return;
    }
    string dat_filename = find_dat_file(start_place.latitude, start_place.longitude,
        end_place.latitude, end_place.longitude, coverage);
    if (dat_filename.empty()) 
    {
        cerr << "No dat file covers both locations. Skipping visualization.\n";
        return;
    }
    dat_data dat;
    if (!load_dat_data(dat_filename, dat)) 
    {
        cerr << "Failed to load dat data. Skipping visualization.\n";
        return;
    }
    double aspect_ratio = (double)dat.columns / dat.rows;
    int win_height = 900;
    int win_width = (int)(win_height * aspect_ratio);
    window* win = make_window(win_width, win_height);
    win->set_caption("Shortest Route Map - " + start_place.name + " to " + end_place.name);
    draw_map(win, dat, win_width, win_height);
    draw_path_on_map(win, path, graph_vec, dat, win_width, win_height);
    draw_markers(win, start_place, end_place, dat, win_width, win_height);
    wait_for(window_close_button);
}
...
int find_nearest_intersection(double latitude, double longitude, const vector<intersection>& graph_vec) 
{
    if (graph_vec.empty())
        return -1;
    int nearest = 0;
    double min_distance = calculate_distance(latitude, longitude, graph_vec[0].latitude, graph_vec[0].longitude);
    for (size_t i = 1; i < graph_vec.size(); i++)
    {
        double dist = calculate_distance(latitude, longitude, graph_vec[i].latitude, graph_vec[i].longitude);
...
void main() 
{
    ...
    cout << "\nFinding shortest path from " << start_place.name << " " << start_place.state_abbreviation
        << " to " << dest_place.name << " " << dest_place.state_abbreviation << "...\n";
    ...
    if (start_intersection_id <= 0 || static_cast<size_t>(start_intersection_id) >= graph_vec.size()) 
    {
        ...
        if (start_intersection_id != -1 && !graph_vec.empty())
        {
            ...
            graph_vec[start_intersection_id].latitude, graph_vec[start_intersection_id].longitude) * 69.0;
        }
    }
    if (end_intersection_id <= 0 || static_cast<size_t>(end_intersection_id) >= graph_vec.size()) 
    {
        ...
        if (end_intersection_id != -1 && !graph_vec.empty()) 
        {
            ...
            graph_vec[end_intersection_id].latitude, graph_vec[end_intersection_id].longitude) * 69.0;
        }
    }
    if (start_intersection_id < 0 || static_cast<size_t>(start_intersection_id) >= graph_vec.size()) 
    {
    ...
    if (reconstruct_path(graph_vec, start_intersection_id, end_intersection_id, path)) 
    {
        ...
        visualize_path(start_place, dest_place, path, graph_vec);
    }
    else
